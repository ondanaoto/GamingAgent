...
def main():
    ...
    parser.add_argument(
        "--model_name", type=str, default="gpt-4-turbo", help="Model name."
    )
    parser.add_argument(
        "--modality",
        type=str,
    ...
    parser.add_argument(
        "--crop_left", type=int, default=880, help="Pixels to crop from the left."
    )
    parser.add_argument(
        "--crop_right", type=int, default=660, help="Pixels to crop from the right."
    )
    parser.add_argument(
        "--crop_top", type=int, default=350, help="Pixels to crop from the top."
    )
    parser.add_argument(
        "--crop_bottom", type=int, default=230, help="Pixels to crop from the bottom."
    )
    parser.add_argument("--grid_rows", type=int, default=8, help="Number of grid rows.")
    parser.add_argument(
        "--grid_cols", type=int, default=8, help="Number of grid columns."
    )
    ...
    try:
        while True:
            ...
            latest_response = candy_crush_worker(
                system_prompt,
                args.api_provider,
                args.model_name,
                args.modality,
                str2bool(args.thinking),
                args.crop_left,
                args.crop_right,
                args.crop_top,
                args.crop_bottom,

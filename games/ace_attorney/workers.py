...
from tools.api_cost_calculator import (
...
def vision_evidence_worker(
    system_prompt, api_provider, model_name, modality, thinking, cache_dir=None
):
    ...
    if api_provider == "anthropic" and modality == "text-only":
        ...
        response = together_ai_completion(
            system_prompt, model_name, prompt, base64_image=base64_image
        )
    else:
    ...
    return {"response": response, "screenshot_path": screenshot_path}
...
def long_term_memory_worker(
    ...
    json_file = os.path.join(
        dialog_history_dir, f"{episode_name.lower().replace(' ', '_')}.json"
    )
    ...
    if os.path.exists(json_file):
        ...
        dialog_history = {episode_name: {"Case_Transcript": [], "evidences": []}}
...
def short_term_memory_worker(
    ...
    if os.path.exists(json_file):
        ...
        dialog_history = {
            episode_name: {"Case_Transcript": [], "prev_responses": [], "evidences": []}
        }
...
def memory_retrieval_worker(
    ...
    memory_file = os.path.join(
        cache_dir, "dialog_history", f"{episode_name.lower().replace(' ', '_')}.json"
    )
...
def reasoning_worker(
    ...
    evidence_details = "\n".join(
        [f"Evidence {i + 1}: {e}" for i, e in enumerate(collected_evidences)]
    )
    # print(scene)
    ...
    """
        ...
        if api_provider == "anthropic" and modality == "text-only":
            ...
            api_provider == "openai" and "o3" in model_name and modality == "text-only"
        ):
            response = openai_text_reasoning_completion(
                system_prompt, model_name, prompt
            )
        elif api_provider == "openai":
            ...
            response = deepseek_text_reasoning_completion(
                system_prompt, model_name, prompt
            )
        elif api_provider == "together_ai":
            response = together_ai_completion(
        ...
        return {"move": move, "thought": thought}

    else:
        time.sleep(1)
        return {"move": "z", "thought": "continue conversation"}


def ace_evidence_worker(
    system_prompt,
    api_provider,
    model_name,
    prev_response="",
    ...
    for name in evidence_names:
        ...
        desc_match = re.search(
            r"Evidence Description:\s*(.+)", vision_result["response"]
        )
        visual_description = (
    ...
    return {"game_state": "Evidence", "collected_evidences": collected}


def ace_attorney_worker(
    system_prompt,
    api_provider,
    model_name,
    prev_response="",
    ...
    game_state_match = re.search(
        r"Game State:\s*(Cross-Examination|Conversation|Evidence)", response_text
    )
    ...
    if options_match:
        ...
        if raw_options.lower() != "none":
            ...
            option_entries = [
                opt.strip() for opt in raw_options.split(";") if opt.strip()
            ]
    ...
    scene_match = re.search(
        r"Scene:\s*((?:.|\n)+?)(?=\n(?:Game State:|Dialog:|Evidence:|Options:|$)|$)",
        response_text,
        re.DOTALL,
    )
    ...
    if decision_state:
        ...
        if reasoning_result["move"] == "z" and decision_state["has_options"]:
            ...
            print(
                f"[Decision Made] Selected option: '{decision_state['selected_text']}' at index {decision_state['selection_index']} (via {decision_state['down_count']} down moves)"
            )
...
def check_skip_conversation(dialog, episode_name):
    ...
    try:
        with open(
            "games/ace_attorney/ace_attorney_1_skip_conversations.json",
            "r",
            encoding="utf-8",
        ) as f:
...
def check_end_statement(dialog, episode_name):
    ...
    try:
        ...
        end_statements = skip_conversations.get(episode_name, {}).get(
            "end_statements", []
        )
...
def vision_only_reasoning_worker(
    system_prompt,
    api_provider,
    model_name,
    prev_response="",
    ...
    game_state_match = re.search(
        r"Game State:\s*(Cross-Examination|Conversation)", response
    )
    game_state = game_state_match.group(1) if game_state_match else "Unknown"
    ...
    scene_match = re.search(
        r"Scene:\s*((?:.|\n)+?)(?=\n(?:Game State:|Dialog:|Evidence:|Options:|move:|thought:|$)|$)",
        response,
        re.DOTALL,
    )
    scene = scene_match.group(1).strip() if scene_match else ""
...
def vision_only_ace_attorney_worker(
    system_prompt,
    api_provider,
    model_name,
    prev_response="",
    ...
    Args:
        ...
        cache_dir (str, optional): Directory to save cache files
    """
    assert modality in ["text-only", "vision-text", "vision-only"], (
        f"modality {modality} is not supported."
    )
    ...
    if "option" in scene.lower() and "selected" in scene.lower():
        ...
        option_lines = [
            ...
            and ("selected" in line.lower() or "highlighted" in line.lower())
        ]

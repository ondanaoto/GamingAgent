...
system_prompt = "You are an expert AI agent specialized in playing Tetris gameplay, search for and execute optimal moves given each game state. Prioritize line clearing over speed."

tetris_board_reader_system_prompt = "You are an expert AI agent specialized in converting a Tetris game grid to a text table."

tetris_board_aggregator_system_prompt = "You are an expert AI agent specialized in aggregating subtable into a bigger text table, please take ptach size offsets into consideration."

speculator_system_prompt = "You are an expert AI agent specialized in playing Tetris gameplay, search for and execute optimal moves given each game state. Prioritize line clearing over speed."
...
def main():
    ...
    parser.add_argument(
        "--speculator_count", type=int, default=1, help="Number of speculators."
    )
    parser.add_argument(
        "--speculation_size",
        type=int,
    ...
    parser.add_argument(
        "--control_time", type=float, default=4, help="Worker control time."
    )
    parser.add_argument(
        "--input_type",
        type=str,
        default="read-from-game-backend",
        help="Game state input type.",
    )
    parser.add_argument(
        "--policy", type=str, default="fixed", choices=["fixed"], help="Worker policy"
    )
    parser.add_argument(
        "--cache_folder",
        type=str,
        default="/Users/lhu/workspace/game_arena_env/Python-Tetris-Game-Pygame/cache/tetris",
        help="game state path.",
    )

    args = parser.parse_args()
    # FIXME (lanxiang): scale the number of speculators
    assert args.speculator_count == 1, (
        f"more than 1 speculator worker ({args.speculator_count} is passed) is not yet supported."
    )
    ...
    offsets = [
        i * (args.control_time + args.concurrency_interval) for i in range(num_threads)
    ]

    num_threads += args.speculator_count

    print(
        f"Starting with {num_threads} threads, ({args.speculator_count} thread[s] are speculator[s]) using policy '{args.policy}'..."
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        if args.speculator_count:
            executor.submit(
                tetris_speculator,
                0,
                args.speculation_delay,
                speculator_system_prompt,
                args.api_provider,
                args.model_name,
                args.speculation_size,
                20,
            )

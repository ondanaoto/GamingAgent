...
def get_best_move(api_provider, model_name, move_history):
    ...
    history_text = (
        "\n".join(
            [
                f"{i + 1}. {entry['move']} -> {entry['thought']}"
    ...
    if api_provider == "anthropic":
        response = anthropic_completion(
            system_prompt, model_name, base64_image, user_prompt
        )
...
def main():
    ...
    parser.add_argument(
        "--game_url",
        type=str,
        default="https://wasm4.org/play/sokoban/#",
        help="Boxxel game URL.",
    )
    parser.add_argument(
        "--auto_open", action="store_true", help="Automatically open the game URL."
    )
    ...
    try:
        while True:
            move, thought = get_best_move(
                args.api_provider, args.model_name, list(move_history)
            )
            if move is not None:
